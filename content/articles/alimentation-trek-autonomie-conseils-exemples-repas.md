...
```tip
title: Astuce organisation : les sacs ziploc
Pour organiser notre nourriture (mais aussi une partie du matériel), nous utilisons de nombreux sacs zip de tailles différentes.

Cela permet plusieurs choses :
	•	garder un sac parfaitement organisé et lisible ;
	•	identifier rapidement le contenu sans tout vider ;
	•	protéger la nourriture de l’humidité ;
	•	répartir les rations par jour ou par type de repas.

Concrètement, nous préparons souvent des sacs “journée” contenant petit-déjeuner, encas et une partie du dîner. Cela rend la gestion de la nourriture beaucoup plus simple une fois sur le terrain, surtout en fin de journée lorsque la fatigue se fait sentir.
```
...
```tip
title: Anticiper les possibilités de ravitaillement
Lorsque l'on prépare un trek en autonomie, le premier réflexe est souvent de calculer la quantité de nourriture nécessaire pour l'ensemble du parcours. Pourtant, il est parfois possible de se réapprovisionner en chemin.

C’est un point qui peut changer complètement la gestion du poids du sac.

Selon les itinéraires, on peut traverser des villages, passer à proximité d'une épicerie, d'une boulangerie ou d'un refuge proposant quelques produits alimentaires. Lorsque c'est possible, cela permet de réduire considérablement le poids porté en début de trek.

Par exemple, sur un itinéraire de six jours, pouvoir effectuer un ravitaillement au bout de trois jours permet de ne transporter que la moitié de la nourriture au départ.

Attention toutefois : il ne faut jamais considérer ces ravitaillements comme acquis sans les avoir vérifiés au préalable. Les horaires d'ouverture peuvent être limités, certains commerces peuvent être fermés hors saison et l'offre disponible est parfois très réduite.

Lors de la préparation de nos itinéraires, nous essayons donc toujours d'identifier les possibilités de ravitaillement, mais nous prévoyons également une marge de sécurité au cas où les choses ne se passeraient pas exactement comme prévu.
```

### Ne pas négliger le plaisir

Après une longue journée de marche, un bon repas peut faire toute la différence.
Lorsque la fatigue s'accumule, que la météo se dégrade ou que le moral baisse, le repas devient souvent un véritable moment de réconfort.
...
layout: 1
maxHeight: 500px
- /content/images/alimentation-trek_002.jpg | Repas du soir après une grosse journée de marche
```
...
Il n'existe pas une seule bonne façon de s'alimenter en trek.
Avec le temps, chacun finit par trouver l'organisation qui lui convient.

```tip
title: Préparer ses rations à l’avance
Nous trouvons très pratique de préparer des rations pour chaque journée avant le départ.
Cette méthode présente plusieurs avantages :
	•	vérifier que l'on dispose de suffisamment de nourriture jusqu'à la fin du trek ;
	•	éviter de consommer trop rapidement les aliments les plus appétissants ;
	•	faciliter l'organisation du sac ;
	•	savoir immédiatement ce qu'il reste à manger pour les jours suivants.
Cela demande un peu de préparation en amont, mais simplifie grandement la gestion quotidienne sur le terrain.
```
...
Pour illustrer la partie théorique, voici un exemple réel de ration alimentaire pour un trek de **6 jours en autonomie complète**, basé sur une organisation simple, économique et relativement optimisée.
L’objectif est d’atteindre environ **15 500 à 16 500 kcal au total**, soit en moyenne environ **2 600 kcal par jour**, tout en limitant le poids porté.
